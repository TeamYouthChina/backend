...
`JOB_DESCRIPTION` VARCHAR(400) COMMENT '职位描述',
`JOB_LOCATION` INTEGER COMMENT '工作地点',
`JOB_HIGHLIGHT` VARCHAR(200) COMMENT '职位亮点',
`DATA_TIME` DATE COMMENT '数据日期'
)COMMENT = '实习推荐表';
...
##短评推荐表
create table IF NOT EXISTS `RECOM_BRIEF_REVIEW`(
`REVIEW_ID` INT COMMENT '短评ID',
`REVIEW_CONTENT` VARCHAR(12000) COMMENT '短评内容',
`DATA_TIME` DATE COMMENT '数据日期'
)COMMENT = '短评推荐表';
