...
`CERTIF_ID` INT AUTO_INCREMENT COMMENT '证书ID',
`CERTIF_NAME` VARCHAR(200) NOT NULL COMMENT '证书名称',
`CERTIF_INSTI` VARCHAR(200) NOT NULL COMMENT '证书颁发机构',
`CERTIF_GRANT_DATE` DATE NOT NULL COMMENT '获得时间',
`CERTIF_EXPIR_DATE` DATE COMMENT '到期时间',
`CERTIF_URL` VARCHAR(200) COMMENT '证书上传地址',
...
create table IF NOT EXISTS `SYS_STU_CONFIGURATION` (
  `STU_ID`                INT AUTO_INCREMENT
  COMMENT '应聘者ID',
  `CONFIG_CV_VISIBLE`     INTEGER NOT NULL DEFAULT '1'
  COMMENT '简历是否可见',
  `CONFIG_RECEIVE_MAIL`   INTEGER NOT NULL DEFAULT '1'
  COMMENT '是否接受应聘邮件邀请',
  `CONFIG_RECEIVE_JOB`    INTEGER NOT NULL DEFAULT '1'
  COMMENT '是否接受推荐职位',
  `CONFIG_RECEIVE_FRIEND` INTEGER NOT NULL DEFAULT '1'
  COMMENT '是否接受好友推荐',
  PRIMARY KEY (`STU_ID`)
)
  COMMENT = '应聘者个人设置表';
