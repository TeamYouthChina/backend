...
##短评表
create table IF NOT EXISTS `COM_BRIEF_COMMENT`(
`COMMENT_ID` INT AUTO_INCREMENT COMMENT '短评ID',
`COMMENT_CONTENT` VARCHAR(200) NOT NULL COMMENT '短评内容',
`COMMENT_TIME` TIMESTAMP NOT NULL COMMENT '短评时间',
`IS_DELETE` INTEGER DEFAULT '0' COMMENT '是否删除',
`IS_DELETE_TIME` TIMESTAMP NULL DEFAULT NULL COMMENT '删除时间',
PRIMARY KEY (`COMMENT_ID`)
)COMMENT = '短评表';
...
##短评映射表
create table IF NOT EXISTS `COM_BRIEF_COMMENT_MAP`(
`VIDEO_ID` INT AUTO_INCREMENT COMMENT '短评ID',
`USER_ID` INTEGER NOT NULL COMMENT '用户ID',
