...
public class CommunityQAServiceImplement implements CommunityQAService {
    ...
    @Override
    @Transactional
    public Attention attentionQuestion(Integer ques_id, Integer user_id) throws NotFoundException{
        getQuestion(ques_id);
        Attention attention = communityQAMapper.isQuestionAttention(ques_id, user_id);
        if(attention == null){
            Attention attention1 = new Attention();
            attention1.setUser_id(user_id);
            communityQAMapper.addAttentionToQuestion(attention1);
            communityQAMapper.createMapBetweenAttentionAndQuestion(ques_id, attention1.getAtten_id());
        }else{
            communityQAMapper.reAddAttentionToQuestion(attention.getAtten_id());
        }
        return attention;
    }

//    private Attention getAttention(Integer atten_id) throws NotFoundException{
//        Attention questionAttention= communityQAMapper.getAttention(atten_id);
...
//    }
    ...
    public void cancelAttentionQuestion(Integer ques_id, Integer user_id) throws NotFoundException{
        ...
        communityQAMapper.cancelAttention(attention.getAtten_id());
    }
    ...
    /**
     * 添加回答，建立回答和问题的映射
     * @param answer 要添加的回答的对象
     * @param ques_id 要回答的问题的id
     * @return 添加成功返回1
     */
    @Override
    @Transactional
    public Answer addAnswer(Answer answer, Integer ques_id, Integer answer_level) throws NotFoundException {
        getQuestion(ques_id);
        communityQAMapper.addAnswerToQuestion(answer);
        communityQAMapper.createMapBetweenQuestionAndAnswer(ques_id, answer.getAnswer_id(), answer_level);
        return communityQAMapper.getAnswerById(answer.getAnswer_id());
    }
     ...
     */
    @Override
    public Answer getAnswer(Integer answer_id) throws NotFoundException{
        Answer answer =  communityQAMapper.getAnswerById(answer_id);
        if(answer == null){
    ...
    private Answer simplyGetAnswer(Integer answer_id) throws NotFoundException{
        Answer answer = communityQAMapper.getAnswer(answer_id);
        if(answer == null){
    ...
    /**
     * 编辑回答，如果回答不存在，抛出异常
     * @param answer 要编辑的回答对象
     * @return 编辑成功返回1
     * @throws NotFoundException
     */
    @Override
    @Transactional
    public Answer editAnswer(Answer answer) throws NotFoundException{
        simplyGetAnswer(answer.getAnswer_id());
     ...
     */
    @Override
    public List<Answer> listMyAnswers(Integer user_id) throws NotFoundException{
        List<Answer> answers = communityQAMapper.listMyAnswer(user_id);
        if(answers == null){
...
//     */
//    @Override
//    public List<Answer> listMyAgreeAnswer(Integer user_id) throws NotFoundException {
//        List<Answer> questionAnswers = communityQAMapper.listMyAgreeAnswer(user_id);
...
//    }
    ...
    public void evaluateComment(Integer comment_id, Integer user_id) throws NotFoundException{
        getComment(comment_id);
        Evaluate evaluate = communityQAMapper.commentEvaluateStatus(user_id, comment_id);
        if(evaluate == null){
            Evaluate evaluate1 = new Evaluate();
            evaluate1.setUser_id(user_id);
            communityQAMapper.addEvaluateToComment(evaluate1);
            communityQAMapper.createMapBetweenCommentAndEvaluate(evaluate1.getEvaluate_id(), comment_id);
        }else {
            communityQAMapper.reEvaluateComment(evaluate.getEvaluate_id());
        }
    }
    ...
    public void deleteEvaluateComment(Integer comment_id, Integer user_id) throws NotFoundException {
        ...
        if(evaluate == null){
            ...
            communityQAMapper.deleteEvaluateComment(evaluate.getEvaluate_id());
        }
    }
...
//     */
//    @Override
//    public Evaluate discussEvaluateStatus(Integer user_id, Integer discuss_id) throws NotFoundException {
//        Evaluate discussEvaluate = communityQAMapper.discussEvaluateStatus(user_id, discuss_id);
...
//    @Override
//    @Transactional
//    public Integer evaluateDiscuss(Integer discuss_id, Evaluate discussEvaluate) throws NotFoundException{
//        getDiscuss(discuss_id);
...
//    }
    ...
    @Override
    @Transactional
    public Attention attentionVideo(Integer user_id, Integer video_id) throws NotFoundException{
        simplyGetVideo(video_id);
        Attention attention = communityQAMapper.videoAttentionStatus(video_id,user_id);
        if(attention == null){
            Attention attention1 = new Attention();
            attention1.setUser_id(user_id);
            communityQAMapper.addAttentionToVideo(attention1);
            communityQAMapper.createMapBetweenAttentionAndVideo(attention1.getAtten_id(), video_id);
        }else{
            communityQAMapper.reAddAttentionToVideo(attention.getAtten_id());
        }
        return attention;
    }
    ...
    public void cancelAttenVideo(Integer user_id, Integer video_id) throws NotFoundException {
        ...
        if(attention == null){
            ...
            communityQAMapper.cancelAttentionVideo(attention.getAtten_id());
        }
    }
    ...
    @Override
    @Transactional
    public Comment commentVideo(Comment comment, Integer video_id, Integer comment_level) throws NotFoundException{
        simplyGetVideo(video_id);
        communityQAMapper.addCommentToVideo(comment);
        communityQAMapper.createMapBetweenCommentAndVideo(comment.getComment_id(), video_id, comment_level);
        return comment;
    }
     ...
     */
    @Override
    public Comment getVideoComment(Integer comment_id) throws NotFoundException{
        Comment comment = communityQAMapper.getVideoComment(comment_id);
        if(comment == null){
    ...
    @Transactional
    @Override
    public Evaluate evaluateVideo(Integer user_id, Integer video_id) throws NotFoundException {
        simplyGetVideo(video_id);
        Evaluate evaluate = communityQAMapper.videoEvaluateStatus(video_id,user_id);
        if(evaluate == null){
            Evaluate evaluate1 = new Evaluate();
            communityQAMapper.addEvaluationToVideo(evaluate1);
            communityQAMapper.createMapBetweenEvaluationAndVideo(evaluate1.getEvaluate_id(), video_id);
        }else{
            communityQAMapper.reEvaluateVideo(evaluate.getEvaluate_id());
        }
        return evaluate;
    }
    ...
    public void cancelEvaluateVideo(Integer user_id, Integer video_id) throws NotFoundException {
        ...
        if(evaluate == null){
            ...
            communityQAMapper.cancelEvaluateVideo(evaluate.getEvaluate_id());
        }
    }
